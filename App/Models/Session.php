...
class Session
{
	...
	public function get(string $key, mixed $default = ''): mixed
	{
		...
		if (isset($_SESSION[$this->userkey][0]->$key)) {
			return $_SESSION[$this->userkey][0]->$key;
		}
	...
	//APPLICATION RELATED UNIQUE  SESSION  FUNCTIONS;

	public static function startApp(array $data): void
	{
	...
	public static function addToApp(array $data,string $customkey = ''){
		
		foreach ($data as $key => $value) {
			if(is_numeric($key)&& !$customkey){
   echo 'A custom  key is required';
			}elseif ($customkey) {
				$_SESSION[$customkey] = $value;
